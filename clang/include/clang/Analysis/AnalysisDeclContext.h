...
public:
  CLANG_ABI AnalysisDeclContext(AnalysisDeclContextManager *Mgr, const Decl *D);

  CLANG_ABI AnalysisDeclContext(AnalysisDeclContextManager *Mgr, const Decl *D,
                      const CFG::BuildOptions &BuildOptions);

  CLANG_ABI ~AnalysisDeclContext();
  ...
  CLANG_ABI void registerForcedBlockExpression(const Stmt *stmt);
  CLANG_ABI const CFGBlock *getBlockForRegisteredExpression(const Stmt *stmt);
  ...
  ///             by the BodyFarm.
  CLANG_ABI Stmt *getBody(bool &IsAutosynthesized) const;
  ...
  /// the scenes.
  /// \sa getBody
  CLANG_ABI bool isBodyAutosynthesized() const;
  ...
  /// the scenes.
  /// \sa getBody
  CLANG_ABI bool isBodyAutosynthesizedFromModelFile() const;

  CLANG_ABI CFG *getCFG();

  CLANG_ABI CFGStmtMap *getCFGStmtMap();

  CLANG_ABI CFGReverseBlockReachabilityAnalysis *getCFGReachablityAnalysis();

  /// \returns A version of the CFG without any edges pruned.
  CLANG_ABI CFG *getUnoptimizedCFG();

  CLANG_ABI void dumpCFG(bool ShowColors);
  ...
  CLANG_ABI ParentMap &getParentMap();

  using referenced_decls_iterator = const VarDecl *const *;

  CLANG_ABI llvm::iterator_range<referenced_decls_iterator>
  ...
  CLANG_ABI const ImplicitParamDecl *getSelfDecl() const;

  /// \copydoc LocationContextManager::getStackFrame()
  CLANG_ABI const StackFrameContext *getStackFrame(LocationContext const *ParentLC,
                                         const Stmt *S, const CFGBlock *Blk,
                                         unsigned BlockCount, unsigned Index);

  /// \copydoc LocationContextManager::getBlockInvocationContext()
  CLANG_ABI const BlockInvocationContext *
  ...
  CLANG_ABI static bool isInStdNamespace(const Decl *D);

  CLANG_ABI static std::string getFunctionName(const Decl *D);

private:
  CLANG_ABI std::unique_ptr<ManagedAnalysis> &getAnalysisImpl(const void *tag);
...
public:
  CLANG_ABI ~LocationContextManager();
  ...
  CLANG_ABI const StackFrameContext *getStackFrame(AnalysisDeclContext *ADC,
...
public:
  CLANG_ABI AnalysisDeclContextManager(
  ...
  CLANG_ABI AnalysisDeclContext *getContext(const Decl *D);
  ...
  CLANG_ABI BodyFarm &getBodyFarm();
